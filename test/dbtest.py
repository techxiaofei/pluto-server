...
if __name__ == "__main__":
	...
	data = {"sql":"select * from player where id = 2;"}
	
	#json_data = json.dumps(data)
	
	print "before send"
	client.sendall(sendData(1, 1, data))   # 用command_id区分query跟update
	print "111111"
	time.sleep(2)
	client.sendall(sendData(2, 0, {"sql":"update player set lv = 5 where id = 2;"}))
	print "222222"
	time.sleep(2)
	client.sendall(sendData(1, 1, data)) 

	for x in range(10):
		#talk(client, userid, x)
		#time.sleep(3)
